  ...
  return (
    <div className="min-h-screen bg-background p-4 pb-20">
      <div className="max-w-4xl mx-auto">
        <div className="bg-card rounded-lg shadow-lg p-6 border border-table-border">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-2xl font-bold text-primary">WhatsApp Export</h1>
              <p className="text-sm text-muted-foreground">Sheet: {sheet} • Range: {RANGE}</p>
            </div>
            <Button onClick={onCopy} disabled={!text} className="shrink-0">Copy</Button>
          </div>
          <Button className="fixed bottom-6 right-6 z-50" onClick={() => navigate("/")}>Home</Button>
          {loading && <div className="text-sm text-muted-foreground">Loading…</div>}
          {error && <div className="text-sm text-destructive">{error}</div>}
          {!loading && !error && (
            <pre className="whitespace-pre-wrap text-sm bg-muted rounded p-4 border border-table-border min-h-[200px]">
              {text || "No data"}
            </pre>
          )}
        </div>
