    ...
    <div className="min-h-screen bg-background p-4 pb-20 overflow-x-hidden">
      <div className="max-w-6xl mx-auto">
        <div className="bg-card rounded-lg shadow-lg p-6 border border-table-border">
          <h1 className="text-2xl font-bold text-primary mb-4">EOD Report Snapshot</h1>
          <p className="text-sm text-muted-foreground mb-4">Sheet: {sheet} • Range: {range}</p>
                ...
                <thead>
                  <tr className="bg-gray-100 dark:bg-gray-800">
                    {Array.from({ length: columns }).map((_, cIdx) => (
                      <th key={cIdx} className="border border-table-border p-2 text-sm text-left font-medium">
                        {values[0]?.[cIdx] ?? ""}
                      </th>
                    ))}
                  </tr>
                    ...
                    return (
                      <tr key={rIdx} className={rowClass}>
                        {Array.from({ length: Math.max(columns, row.length) }).map((_, cIdx) => (
                          <td key={cIdx} className="border border-table-border p-2 text-sm">
                            {row[cIdx] ?? ""}
                          </td>
                        ))}
                      </tr>
